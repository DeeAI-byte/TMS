...
st.sidebar.write("---")
st.sidebar.header("🚛 Route Consolidation")
st.sidebar.caption("Per-distributor truck counts assume one dedicated truck per distributor per day, which "
                    "doesn't account for one truck serving multiple nearby stops on a route. Validated against "
                    "June actuals: raw per-distributor math was **2.07x** higher than real fleet usage.")
consolidation_factor_pct = st.sidebar.slider(
    "Route consolidation factor (%)", 20, 100, 48, 1, key="consolidation_daily",
    help="Multiplies the raw per-distributor truck total down to reflect real route consolidation. "
         "48% is calibrated from June actuals — adjust as more months validate it."
)
st.sidebar.caption("Defaults below are calibrated against real June actuals (see Methodology).")
own_registered = st.sidebar.number_input("Total Own Fleet — registered (trucks)", min_value=0, value=own_total_default, step=1, key="own_daily")
own_util_pct = st.sidebar.slider("Own fleet — effective utilization %", 0, 100, 90, 5, key="own_util_daily",
                                  help="Not every registered truck is genuinely poolable every day (maintenance, driver availability, etc). June actuals showed ~90% for Own.")
own_return_rate_pct = st.sidebar.slider("Own — % back the NEXT day", 0, 100, 50, 5, key="rr_daily")
own_tat_days = st.sidebar.number_input("Own — full turn-around time (days)", min_value=1, max_value=14, value=2, step=1, key="tat_daily")

st.sidebar.write("")
fixed_registered = st.sidebar.number_input("Total Fixed/Bachat Fleet — registered (trucks)", min_value=0, value=fixed_total_default, step=1, key="fixed_daily")
fixed_util_pct = st.sidebar.slider("Fixed/Bachat — effective utilization %", 0, 100, 70, 5, key="fixed_util_daily",
                                    help="Fixed/Bachat trucks are often tied to dedicated routes/contracts, so fewer are generically poolable. June actuals showed ~70%.")
fixed_return_rate_pct = st.sidebar.slider("Fixed/Bachat — % back the NEXT day", 0, 100, 30, 5, key="rr_fixed_daily")
fixed_tat_days = st.sidebar.number_input("Fixed/Bachat — full turn-around time (days)", min_value=1, max_value=14, value=2, step=1, key="tat_fixed_daily")
...
k3.metric("Trucks Needed / Day (after consolidation)", f"{int(total_trucks_per_day):,}",
          help=f"Raw per-distributor total was {int(total_trucks_per_day_raw):,}/day, "
               f"scaled by the {consolidation_factor_pct}% route consolidation factor.")
k4.metric("Avg Truck Utilization", f"{int(round(avg_util*100))}%" if pd.notna(avg_util) else "—")
st.caption(f"ℹ️ Raw per-distributor calculation: **{int(total_trucks_per_day_raw):,} trucks/day** → after "
           f"**{consolidation_factor_pct}%** route consolidation factor: **{int(total_trucks_per_day):,} trucks/day** "
           f"(this adjusted figure feeds the Fleet Allocation section below).")
...
default_alloc_rows = simulate_daily_allocation(
    default_daily_requirements, int(own_total), int(fixed_total),
...
with st.expander("🛠️ Advanced — customize day-by-day requirement (if daily load actually varies)"):
    ...
    alloc_rows = simulate_daily_allocation(
        daily_requirements, int(own_total), int(fixed_total),
        return_rate=own_return_rate_pct / 100.0, tat_days=int(own_tat_days),
        fixed_return_rate=fixed_return_rate_pct / 100.0, fixed_tat_days=int(fixed_tat_days)
    )
    ...
    n_days_a = len(alloc_df)
    avg_own_a = round(alloc_df["Own Used"].mean()) if n_days_a else 0
    avg_fixed_a = round(alloc_df["Fixed Used"].mean()) if n_days_a else 0
    avg_spot_a = round(alloc_df["Spot Hire Used"].mean()) if n_days_a else 0
    total_spot_a = int(alloc_df["Spot Hire Used"].sum())

    st.caption("Averages per day across this custom table (matches the units in the summary card above).")
    a1, a2, a3, a4 = st.columns(4)
    a1.metric("Own Fleet / Day (avg)", f"{avg_own_a:,}")
    a2.metric("Fixed/Bachat / Day (avg)", f"{avg_fixed_a:,}")
    a3.metric("Spot Hire / Day (avg)", f"{avg_spot_a:,}")
    a4.metric("Spot Hire — Total This Month", f"{total_spot_a:,}")
...
with st.expander("ℹ️ Allocation Methodology"):
    ...
    **{fixed_return_rate_pct}%**/**{fixed_tat_days} days** — Own and Fixed are modeled independently since
    June actuals showed they behave differently. A distributor's demand on Day 2 is first covered by trucks
