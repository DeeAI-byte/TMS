...
st.sidebar.caption("Per-distributor truck counts assume one dedicated truck per distributor visit, which "
                    "doesn't account for one truck serving multiple nearby stops. Validated against June "
                    "actuals: raw per-distributor math was **1.43x** higher than real fleet usage.")
consolidation_factor_pct = st.sidebar.slider(
    "Route consolidation factor (%)", 20, 100, 70, 1, key="consolidation_freq",
    help="Multiplies the raw per-distributor schedule down to reflect real route consolidation. "
...
raw_schedule_f = compute_frequency_daily_schedule(f, working_days_per_week, weeks_in_month)
if len(raw_schedule_f) == 0:
    raw_schedule_f = [0] * working_days_in_month
default_daily_requirements_f = [int(round(x * consolidation_factor_pct / 100)) for x in raw_schedule_f]

st.caption(f"ℹ️ Raw per-distributor schedule averaged **{round(sum(raw_schedule_f)/len(raw_schedule_f)):,}/day** → "
           f"after **{consolidation_factor_pct}%** route consolidation factor: "
           f"**{round(sum(default_daily_requirements_f)/len(default_daily_requirements_f)):,}/day** "
           f"(this adjusted figure feeds everything below).")
