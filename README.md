...
The Own/Fixed/Spot Hire split defaults (effective utilization %, TAT return rates, truck
capacity) are tuned against your actual June performance data, not guessed:
...
Key findings baked into the defaults:
...
- **Own and Fixed have independent TAT behavior** — Own returns faster (~50%/next day, full by day 2) than Fixed/Bachat (~30%/next day), which is now modeled separately instead of one shared assumption.

All of these are still fully editable in the sidebar — as you get more months of actuals, re-run this same validation to refine them further.
