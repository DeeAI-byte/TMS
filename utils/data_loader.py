...
def format_truck_size(value):
    """Formats a raw tonnage number (e.g. 18, 18.0) as a Truck Size label (e.g. '18T'),
    so tables never show a bare capacity number that could be confused with a shipment's
    own Load (Ton) figure. Blank/NaN stays blank."""
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return ""
    try:
        return f"{float(value):g}T"
...
# --------------------------------------------------------------------------------------
# DISPATCH STATUS — Pending / Allotted / Dispatched, per shipment. Once a shipment is
# Allotted, its suggested vehicle is FIXED (locked) so it can't be re-suggested to a
# different shipment on the next rerun. Once Dispatched, that vehicle is logged into the
# Vehicle Out gate-out log automatically.
# --------------------------------------------------------------------------------------
def load_allocation_state():
    os.makedirs(LOCAL_DATA_DIR, exist_ok=True)
    if os.path.exists(ALLOCATION_STATE_PATH):
        try:
            df = pd.read_csv(ALLOCATION_STATE_PATH, dtype=str).fillna("")
