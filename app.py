...
k2.metric(f"{sel_month.title()} Volume (cases)", f"{total_month_volume:,.0f}")
k3.metric("Own Trucks", f"{total_own_trucks}")
k4.metric("Fixed/3rd-Party Trucks", f"{total_fixed_trucks}")
k5.metric("Total Fleet Capacity (T)", f"{total_fleet_tonnage:,.1f}")
k6.metric("Avg Max Vehicle Allowed (T)", f"{avg_max_vehicle:,.1f}" if pd.notna(avg_max_vehicle) else "—")
...
st.dataframe(
    f[available_cols].sort_values(sel_month, ascending=False, na_position="last"),
